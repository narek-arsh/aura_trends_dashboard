...
# ─────────────────────────────────────────────────────────────────────────────
# Carga de claves (soporta varias)
# Preferencia:
#   - GEMINI_API_KEYS = "KEY1,KEY2,KEY3"
#   - (fallback) GEMINI_API_KEY = "KEY_UNICA"
# ─────────────────────────────────────────────────────────────────────────────
...
def _pause_seconds() -> float:
    ...
    Prioridades:
      1) GEMINI_SECONDS_PER_CALL (float, segs por petición)
      2) GEMINI_RPM_PER_KEY (int/float, req por min por clave) -> segs = 60/rpm
      3) DEFAULT = 8.0 seg (≈7.5 rpm por clave, muy conservador)
    """
    # 1) segundos directos
    ...
    if val:
        try:
            s = float(val)
            return max(s, 2.0)  # mínimo 2s por seguridad
        except:
            pass
